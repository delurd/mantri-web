...
export const cekIsSensorOnline = (time: string) => {
    ...
    return { data: false, gapTime: calculateGap, timeDb: parseInt(moment(new Date()).utcOffset(7).format('X')), timeDbs: moment(new Date()).utcOffset(7).format('HH:mm:ss'), timeSensor: parseInt(moment(time, 'HH:mm:ss').format('X')), timeSensors: time }
}